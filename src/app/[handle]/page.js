...
export default function InstructorProfile() {
  ...
  if (!instructor) return (
    <div className="min-h-screen bg-[#FAF9F6] flex flex-col items-center justify-center text-center p-6">
      <h2 className="text-2xl font-bold text-[#2C2A28] mb-2">Profile Not Found</h2>
      <p className="text-[#7A7571]">This instructor hasn't set up their schedule yet.</p>
    </div>
  );

  return (
    <div className="min-h-screen bg-[#FAF9F6] text-[#2C2A28] pb-20">
      
      {/* BIO HEADER */}
      <div className="bg-white border-b border-[#E8E6E1] py-12 px-6 text-center">
        <div className="w-20 h-20 bg-[#F3F0EA] rounded-full mx-auto mb-4 flex items-center justify-center border border-[#E8E6E1]">
          <span className="text-2xl font-bold text-[#7A7571]">
            {instructor.full_name?.charAt(0) || 'I'}
          </span>
        </div>
        <h1 className="text-3xl font-bold tracking-tight">{instructor.full_name}</h1>
        <p className="text-[#7A7571] max-w-md mx-auto mt-3 text-sm leading-relaxed">
              ...
              <div key={c.id} className="bg-white rounded-xl shadow-sm border border-[#E8E6E1] p-5 sm:p-6 flex flex-col sm:flex-row sm:justify-between sm:items-center transition-all">
                <div className="mb-4 sm:mb-0">
                  <div className="flex items-center space-x-3 mb-1.5">
                    ...
                    <span className="text-xs font-medium bg-[#F3F0EA] px-2 py-0.5 rounded border border-[#E8E6E1]">
                      {c.class_type}
                    </span>
                  </div>
                ...
                {c.is_waitlisted ? (
                  <button 
                    ...
                    className="w-full sm:w-auto bg-[#E8E6E1] text-[#7A7571] font-bold py-3.5 px-8 rounded-lg hover:bg-[#D4D2CD] hover:text-[#2C2A28] active:scale-[0.98] transition-all"
                  >
                    Join Waitlist
                  </button>
                ) : (
                  <button 
                    onClick={() => handleBookClick(c)}
                    className="w-full sm:w-auto bg-[#2C2A28] text-white font-bold py-3.5 px-8 rounded-lg hover:bg-[#4A4744] active:scale-[0.98] transition-all"
                  >
                    Book Spot
                  </button>
                )}
